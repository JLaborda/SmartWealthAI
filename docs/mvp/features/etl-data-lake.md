...
- SimFin fundamentals normalizer in `src/smartwealthai/simfin_normalizer.py` and
  ...
  `tests/fixtures/lake/raw/simfin/`. Curated writes preserve existing PIT rows
  in a `(cik, period)` partition and de-duplicate by
  `(cik, fiscal_period_end, as_of_date, version_id)`.
