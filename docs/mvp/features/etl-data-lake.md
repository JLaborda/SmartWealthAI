...
5. The normalizer stamps every fundamentals row with `as_of_date` (EDGAR acceptance timestamp), `fiscal_period_end`, `metric`, and `version_id` (monotonic per CIK + metric + fiscal period to track restatements).
