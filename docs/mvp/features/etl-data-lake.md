...
5. The normalizer writes fundamentals in long form and stamps every value row with `metric`, `as_of_date` (EDGAR acceptance timestamp), `fiscal_period_end`, and `version_id` (monotonic per CIK + metric + fiscal period to track restatements).
...
| Restated values produce a new `version_id` for the same `(cik, metric, fiscal_period_end)` | Warn |
...
- Every curated fundamentals row has `(cik, metric, fiscal_period_end, as_of_date, version_id)` as the natural key.
- A query "fundamentals as of decision date D" returns, per `(cik, metric, fiscal_period_end)`, the row with the highest `as_of_date <= D` and, on tie, the highest `version_id`.
...
- The fixture PIT helper preserves multiple long-form metrics for the same
  company and fiscal period instead of collapsing them into a single row.
