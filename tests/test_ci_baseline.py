...
def test_point_in_time_query_preserves_all_metrics_for_same_period(monkeypatch) -> None:
    ...
    assert set(snapshot["metric"]) == {"OperatingIncomeLoss", "NetIncomeLoss"}
    assert len(snapshot) == 2
    assert (snapshot["version_id"] == 1).all()
