...
- This PRD captures decisions from the CI/CD design session (grill-me). It should be reflected in a future feature spec under `spec/features/` (e.g. `cicd-infrastructure.md`) and cross-linked from `spec/constitution/mission.md` when implementation starts — per spec-driven workflow, the feature spec becomes canonical for acceptance criteria and implementation status.
...
- When implementation begins, update `spec/README.md` to index this PRD under an MVP PRDs section alongside the devcontainer PRD.
- GitHub issue creation with label `ready-for-agent` is recommended for tracking vertical implementation slices (`to-issues`), but this document is the saved PRD artifact at `spec/prds/ci-cd/ci-cd-prd.md` as requested.
